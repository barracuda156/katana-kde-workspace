...
void RandROutput::queryOutputInfo(void)
{
    ...
    kDebug() << "XID" << m_id << "is output" << m_name << (isConnected() ? "(connected)" : "(disconnected)");
...
void RandROutput::load(KConfig &config)
{
    ...
    if (!m_screen->outputsUnified() || m_screen->connectedCount() <= 1) {
        m_proposedRect = (
            cg.readEntry("Rect", "0,0,0,0") == "0,0,0,0")
...
QStringList RandROutput::startupCommands() const
{
    ...
    if (!m_screen->outputsUnified() || m_screen->connectedCount() <= 1) {
        ...
        const QSize modeSize = m_crtc->rect().size();
        // NOTE: the mode has to be one of the modes X11 server knows about
